...
if Gem::Version.new(RUBY_VERSION) < Gem::Version.new('1.9')
  warn 'This script requires Ruby >= 1.9'
  exit 1
end
...
unless result[2].exitstatus == 0
  warn 'You do not appear to be in a git repository. This script must be run from inside a git repository.'
  exit 2
end
filename = result[0].lines.first.chomp + '/CHANGES.md'
unless File.exist?(filename)
  warn 'CHANGES.md not found. Please ensure that CHANGES.md exists.'
  exit 3
end
...
unless versions[version]
  warn "Version #{version} is invalid. Valid versions are: #{versions.keys.join(', ')}"
  exit 4
end
...
unless result[2].exitstatus == 0
  warn "Commit '#{commit}' is not valid."
...
def word_wrap(line, width)
  ...
  line[prefix.size..-1].gsub(/(^)?(.{1,#{real_width}})(?: +|$)/) do |s|
    Regexp.last_match(1) ? "#{first_prefix}#{s}\n" : "#{prefix}#{s}\n"
  end
end
