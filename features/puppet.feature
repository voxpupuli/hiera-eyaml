Feature: eyaml hiera integration
  ...
  Scenario: verify puppet3 with hiera can use hiera-eyaml to decrypt data
    ...
    When I run `puppet apply --confdir ./puppet --node_name_value localhost puppet/manifests/init.pp 2>&1`
  ...
  Scenario: verify puppet3 and facter for correct hash merge with incorrect fact
    ...
    When I run `puppet apply --confdir ./puppet-hiera-merge --node_name_value localhost puppet-hiera-merge/manifests/init.pp 2>&1`
    Then the file "/tmp/eyaml_puppettest.1" should match /^good night$/
