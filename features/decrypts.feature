Feature: eyaml decrypting
  ...
  Scenario: decrypt a file using a temporary keyfile
    When I run `./decrypt_with_key_from_env.sh test_multiple.yaml`
    Then the output should match /one: DEC::PKCS7\[hello\]\!/
    And the output should match /two: DEC::PKCS7\[world\]\!/
